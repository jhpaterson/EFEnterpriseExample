...
using EnterpriseExample.Payroll.Data.Repositories;
using EnterpriseExample.Payroll.Data.Context;

namespace EnterpriseExample.MVC4.DependencyResolution {
            ...
            ObjectFactory.Initialize(x =>
                        {
                            ...
                            x.For<IEmployeeRepository>().Use<EmployeeRepository>();
                            x.For<IDepartmentRepository>().Use<DepartmentRepository>();
                            x.For<IHRUnitOfWork>().HttpContextScoped().Use<HRContext>();
                            x.For<IPayrollUnitOfWork>().HttpContextScoped().Use<PayrollContext>();
                        });
